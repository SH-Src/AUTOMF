...
def convert_to_3digit_icd9(dxStr):
    if dxStr.startswith('E') or dxStr.startswith('V'):
...
class MyDataset(Dataset):
    def __init__(self, x1, x2, s, input_ids, token_type_ids, attention_mask, label, device):
        ...
        self.label = label
        self.device = device
        self.dict_code = pickle.load(open('./data/diagnosis_codes.pkl', 'rb'))
    ...
    def __getitem__(self, idx):
        if torch.is_tensor(idx):
            idx = idx.tolist()
        out_vector = np.zeros(len(self.dict_code))
        diag_label = self.label[idx]
        code_list = diag_label.strip().split(',')
        for code in code_list:
            code_ = convert_to_3digit_icd9(code)
            out_vector[self.dict_code[code_]] = 1.0
        return torch.FloatTensor(self.x1[idx]).to(self.device), torch.FloatTensor(self.x2[idx]).to(self.device), \
               ...
               torch.tensor(out_vector, dtype=torch.float).to(self.device)
...
class DataModule(object):
    ...
    def split(self):
        ...
        diagnosis = pd.read_csv(self.mimic_dir + '/' + 'DIAGNOSES_ICD.csv')
        icus = pd.read_csv(self.mimic_dir + '/' + 'ICUSTAYS.csv', parse_dates=['INTIME', 'OUTTIME']).sort_values(
            by=['SUBJECT_ID']).reset_index(drop=True)
        df = pd.merge(icus[['SUBJECT_ID', 'HADM_ID', 'ICUSTAY_ID', 'INTIME']], diagnosis, on=['SUBJECT_ID', 'HADM_ID'],
                      how='inner')
        df = df.drop(columns=['SUBJECT_ID', 'HADM_ID'])
        df = pd.merge(df_label['ICUSTAY_ID'], df, on='ICUSTAY_ID', how='left')

        df_diagnosis = df.groupby('ICUSTAY_ID')[
            'ICD9_CODE'].apply(lambda x: ','.join(x)).reset_index()
        df_label = pd.merge(df_label, df_diagnosis, on='ICUSTAY_ID', how='left')

        df_label = df_label[df_label['ICD9_CODE'].notnull()]
        ...
        diag_tr = df_label.loc[train_idx]['ICD9_CODE'].tolist()
        diag_d = df_label.loc[val_idx]['ICD9_CODE'].tolist()
        diag_t = df_label.loc[test_idx]['ICD9_CODE'].tolist()

        trainset = MyDataset(X1_train, X2_train, s_train, input_ids_tr, token_type_ids_tr, attention_mask_tr, diag_tr, self.device)
        valset = MyDataset(X1_val, X2_val, s_val, input_ids_d, token_type_ids_d, attention_mask_d, diag_d, self.device)
        testset = MyDataset(X1_test, X2_test, s_test, input_ids_t, token_type_ids_t, attention_mask_t, diag_t, self.device)
