...
OPS1 = {
    ...
    'interaction_3': lambda d_model: Catfc(d_model)
}
...
OPS4 = {
    ...
    'att': lambda d_model: Attention(d_model),
    'mlp': lambda d_model: MLP(d_model),
}
...
class Zero(nn.Module):
    ...
    def forward(self, x, masks, lengths, other):
...
class MLP(nn.Module):
    def __init__(self, d_model):
        ...
        self.relu = nn.ReLU()
        self.layernorm = nn.LayerNorm(d_model)
        self.linear = nn.Sequential(nn.Linear(d_model, 4*d_model), nn.ReLU(), nn.Linear(4*d_model, d_model))
    def forward(self, all_x):
        out = torch.stack(all_x, dim=0)
        out = torch.sum(out, dim=0)
        return self.layernorm(self.linear(out))
...
class Catfc(nn.Module):
    ...
    def forward(self, s, x1, x2, s2):
        s_ = torch.cat((s, s2), dim=-1)
        return self.layer_norm(self.dropout(self.ffn(s_)))
...
class Attention(nn.Module):
    def __init__(self, in_feature, num_head=1, dropout=0.1):
        super(Attention, self).__init__()
        self.linear = nn.Linear(in_feature, 1)
        self.relu = nn.ReLU()
        self.layernorm = nn.LayerNorm(in_feature)
    ...
    def forward(self, x):
        ...
        out = x * weights
        return self.layernorm(out.sum(1))


class SelfAttention(nn.Module):
    def __init__(self, in_feature, num_head=2, dropout=0.1):
    ...
    def forward(self, x, attn_mask, lengths, other):
        batch_size = x.size(0)
        res = x
...
class Attention_x(nn.Module):
    ...
    def forward(self, current_x, s, other_x, s2):
...
class Bert(nn.Module):
    def __init__(self,
                 pretrained_bert_dir: str = './ClinicalBERT_checkpoint/ClinicalBERT_pretraining_pytorch_checkpoint/pytorch_model.bin',
                 freeze: tuple = ()):
        super().__init__()
        self.bert = BertModel(config=BertConfig(), add_pooling_layer=False)
    ...
    def forward(self, input_ids, token_types, attention_mask):
        return self.pooler(self.bert(input_ids=input_ids, token_type_ids=token_types, attention_mask=attention_mask)[0])